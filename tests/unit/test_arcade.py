from arcade_protocol.arcade import (
    Arcade,
)
...
def test_new_payment_code(mocker):
    random_value = '0x1234'
    mocker.patch(
        'arcade_protocol.arcade.random_32bytes',
...
def test_confirm_payment_success(contract, game, user, owner):
    contract.add_game(game.id, PRICE, PERCENT_FEE, from_addr=owner)
    contract.pay(game.id, PAYMENT_CODE, value=PRICE, from_addr=user)
    arcade = Arcade(contract, game, player=user.address)
...
def test_confirm_payment_user_hasnt_uploaded_payment_code(
    ...
    contract.pay(game.id, old_payment_code, value=PRICE, from_addr=user)
    arcade = Arcade(contract, game, player=user.address)
    error = arcade.confirm_payment(
        PAYMENT_CODE,
    )
