...
def test_jackpot(contract, owner, user):
    ...
    expected_jackpot = 0
    key = owner.key
    sig = sign_score(key, contract.address, GAME_ID, user.address, SCORE)
...
def test_user_is_not_signed_user(contract, owner, user, user2):
    ...
    contract.pay(GAME_ID, PAYMENT_CODE, value=PRICE, from_addr=user)
    key = owner.key
    sig = sign_score(key, contract.address, GAME_ID, user2.address, SCORE)
    vrs = (sig['v'], sig['r'], sig['s'])
    receipt = contract.claim_highscore(GAME_ID, SCORE, vrs, from_addr=user)
    assert receipt['status'] == 0
...
def test_score_too_low(contract, owner, user):
    ...
    low_score = 0
    key = owner.key
    sig = sign_score(key, contract.address, GAME_ID, user.address, low_score)
    vrs = (sig['v'], sig['r'], sig['s'])
